...
type Props = {
  craftman: any; //ICraftman2
  randomColor: string;
};

function CraftmanCalendarItem(props: Props) {
  const { craftman, randomColor } = props;
  const { craftman_name, craftman_image, craftman_domaine, craftman_color } =
    craftman;
  return (
    <div
      className="flex items-center gap-2 h-14 bg-white rounded-xl pl-6 max-w-[225px] border-2 border-solid"
      style={{ borderColor: craftman_color }}
    >
