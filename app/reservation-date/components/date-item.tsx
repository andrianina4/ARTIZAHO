    ...
    return (
        <div className='flex justify-between items-center mb-8'>
            <div className='flex items-center'>
                ...
                <p className='bg-reddishBrown text-white font-semibold py-2 px-4 rounded-lg mr-2 w-40 flex items-center justify-center gap-x-2'><span className='text-base'><People /></span> {remainingSpace} person{remainingSpace > 1 ? 's' : ''}</p>
            </div>

            <Button onClick={redirectValidation} content='CHOOSE' className={`w-min h-min ${isSelected ? '' : 'opacity-50'}`} />
        </div>
    )
}
